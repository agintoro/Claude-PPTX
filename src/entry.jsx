/* Entry point for esbuild bundle — renders a single slide at native resolution */
import React from 'react';
import ReactDOM from 'react-dom/client';

// Inline the slide + theme code (converted from the window.* pattern)
// ─────────────────────────────────────────────────────────────────────────────
    ...
    case 'benefits':
      return (
        <div>
          ...
          <p style={{ ...BODY(theme, 24), marginTop: 30, marginBottom: 30, color: theme.muted, fontWeight: 700 }}>{slide.intro}</p>
...
function Slide({ slide, direction, theme, headlineScale }) {
  ...
  return (
    <div style={{ position: 'relative', width: 1080, height: 1350, background: theme.surface, color: theme.ink, overflow: 'hidden', fontFamily: '"Inter", ui-sans-serif, system-ui, sans-serif', WebkitFontSmoothing: 'antialiased' }}>
...
// ── Boot: read params injected by puppeteer ────────────────────────────────
const params = window.__SLIDE_PARAMS__ || { slideIndex: 0, direction: 'A', bgTone: 'ink', headlineScale: 0.98 };
const theme  = getTheme(params.direction, params.bgTone);
const slide  = SLIDES[params.slideIndex];

ReactDOM.createRoot(document.getElementById('root')).render(
  <Slide slide={slide} direction={params.direction} theme={theme} headlineScale={params.headlineScale} />
);
